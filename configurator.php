...
$totalPrice = 0;
$errors = [];
$saveMessage = ''; // Сообщение о сохранении
    ...
    if (isset($_POST['save_build'])) {
        ...
        if (empty($errors)) {
            ...
            foreach ($selectedComponents as $comp) {
                $componentsForSave[] = ['id' => $comp['id'], 'name' => $comp['name']];
            }
            $componentsJson = json_encode($componentsForSave, JSON_UNESCAPED_UNICODE);

            $stmt = $pdo->prepare("INSERT INTO builds (name, components, total_price, created_at) VALUES (?, ?, ?, NOW())");
            try {
